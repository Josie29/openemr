...
> **⚠️ Partially superseded — read this first.** The **UI placement in Section B (the
> chat panel as a Dashboard card mounted via `PatientDemographics\RenderEvent`) is
> replaced** by the docked-sidebar design in `context/specs/copilot-sidebar.md`
> (executed by `implementation-prompt-04-copilot-sidebar.md`). Do **not** rebuild the
> card from this document. Everything else here is **still current and load-bearing**:
> the SMART EHR-launch flow (`launch.php`/`callback.php`/token exchange), the agent-side
> CORS + per-request-token edits, the module skeleton, the OAuth client
> registration/enable/skip-consent admin steps, and the core `csrf` fix. The sidebar
> rework reuses the launch/token flow unchanged. Keep this file as the record of how that
