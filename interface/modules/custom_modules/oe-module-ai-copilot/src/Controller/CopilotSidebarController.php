...
final readonly class CopilotSidebarController
{
     ...
     * question, and two synthesis questions that pair extracted document facts with matching
     * guidance. The document prompts assume an uploaded document is on file (the demo patient has a
     * lab report and an intake form); for a patient without one the agent reports that none is on
     * file.
     *
     * @var list<string>
     */
    private const STARTER_PROMPTS = [
        ...
        // Synthesis across EVERY uploaded document, not just the labs — the prompt that exercises
        // multi-document extraction (a lab report and an intake form in one turn) and lands them as
        // separate provenance cards. Deliberately says "the documents", not "the lab report": the
        // agent discovers what is on file rather than being told what to read.
