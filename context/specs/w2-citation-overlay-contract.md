...
## Why a session-authed viewer for the browser overlay
The **in-browser overlay** is served by the **session-authenticated `source-view.php`** (the logged-in EHR user's own session ACL, reading the document by uuid via the core document model). This keeps the *browser* rendering path free of any SMART Binary scope — the producer only stamps the citation fields.

> **Correction (later work):** an earlier version of this note claimed a patient-launch SMART app "cannot obtain" Binary read scope. That was wrong — OpenEMR generates a `patient/Binary.read` scope (`ServerScopeListEntity`), and the `GET /fhir/Binary/:id` route serves patient requests. The **agent** now fetches document bytes server-side over FHIR `Binary` with that scope (see below); the browser viewer still uses the session for rendering.
...
- ~~**Production doc-bytes fetch**~~ **(DONE)** — the extractor now fetches bytes from OpenEMR by id over FHIR `Binary` (`FhirBinaryByteSource` → `GET /Binary/{document_id}`) on the request's own patient-scoped token, keyed on the same UUID the citation + viewer use. This required (a) granting `patient/Binary.read` and (b) fixing OpenEMR core so a patient-scoped request authorizes documents by patient ownership (`can_patient_access`) instead of the clinician category ACL (`can_access`) — which also unblocked `DocumentReference` discovery of Documents-tab uploads. The fixture FHIR client serves a committed PDF for the same path offline.
