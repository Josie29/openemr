...
def test_golden_set_size() -> None:
    ...
    # that lands with JOS-82), plus angulo-uploaded-med-interaction (the AF-VULN-0001 regression:
    # an unreconciled document med must not drive a verified interaction warning). A change to the
    # count means the matrix allocation drifted and coverage should be re-reviewed — swapping a case
    # in and out keeps this green, which is intended; the ground-truth and falsifiability checks
    # above are what police the swap.
    assert len(CASES) == 54
